...
namespace Utility
{
    ...
    {
        public Iterator(List<TIterable> collection, IterationOptions option) : this(collection)
        {
            iterationOption = option;
        }
        ...
        public void MoveForward()
        {
            ...
            {
                if (iterationOption is IterationOptions.KeepIterating) ResetIndex();
        ...
        public TIterable LookFromCurrentBy(int position, Direction direction)
        {
            var lookAtPosition = direction is Direction.Forward ?
                CurrentIndex + position : CurrentIndex - position;

            return lookAtPosition < CollectionLength + 1 && lookAtPosition > 0
                ? collection[lookAtPosition] : default;
        }
